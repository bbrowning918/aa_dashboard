...
describe("ippSlice tests", () => {
    ...
    test("nextTurn updates if all powers have saved", () => {
        const state = {
            [Powers.GERMANY]: {
                0: { start: 20, spent: 20, income: 20 },
                1: { start: 20 },
            },
            [Powers.SOVIET_UNION]: {
                0: { start: 8, spent: 8, income: 8 },
                1: { start: 8 },
            },
            [Powers.COMMUNIST_CHINA]: {
                0: { start: 2, spent: 2, income: 2 },
                1: { start: 2 },
            },
            [Powers.JAPAN]: {
                0: { start: 16, spent: 16, income: 16 },
                1: { start: 16 },
            },
            [Powers.UK_WEST]: {
                0: { start: 11, spent: 11, income: 11 },
                1: { start: 11 },
            },
            [Powers.UK_EAST]: {
                0: { start: 5, spent: 5, income: 5 },
                1: { start: 5 },
            },
        ...
        expect(ippReducer(state, nextTurn())).toEqual({
            [Powers.GERMANY]: {
                0: { start: 20, spent: 20, income: 20 },
                1: { start: 20 },
            },
    ...
    test("nextTurn no effect if not all powers have saved", () => {
        ...
        expect(ippReducer(state, nextTurn())).toEqual(state);
    });
    test("nextPower updates when not at end", () => {
        ...
        expect(ippReducer(state, nextPower())).toEqual({
            [Powers.GERMANY]: { 0: { start: 20 } },
            [Powers.SOVIET_UNION]: { 0: { start: 8 } },
            [Powers.COMMUNIST_CHINA]: { 0: { start: 2 } },
            [Powers.JAPAN]: { 0: { start: 16 } },
    ...
    test("nextPower no effect when at end", () => {
        ...
        expect(ippReducer(state, nextPower())).toEqual(state);
    });
    test("prevPower updates when not at start", () => {
        ...
        expect(ippReducer(state, prevPower())).toEqual({
            [Powers.GERMANY]: { 0: { start: 20 } },
            [Powers.SOVIET_UNION]: { 0: { start: 8 } },
            [Powers.COMMUNIST_CHINA]: { 0: { start: 2 } },
            [Powers.JAPAN]: { 0: { start: 16 } },
    ...
    test("saveCurrent updates this turn and sets up next", () => {
        ...
        expect(ippReducer(state, saveCurrent({ spent: 15, income: 20 }))).toEqual({
            [Powers.GERMANY]: {
                0: { start: 20, spent: 15, income: 20 },
                1: { start: 25 },
            },
            [Powers.SOVIET_UNION]: { 0: { start: 8 } },
            [Powers.COMMUNIST_CHINA]: { 0: { start: 2 } },
